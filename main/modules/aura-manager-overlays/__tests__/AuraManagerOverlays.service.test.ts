...
function createFakeWindow(
  ...
  return {
    ...
    showInactive: vi.fn(),
    webContents: {
      getURL: vi.fn(() => `app://-/${WindowName.AuraOverlay}`),
...
describe("AuraManagerOverlaysService", () => {
  it("shows one click-through full-display aura overlay for all placements", async () => {
    ...
    service.previewPlacement(profile.id, {
      id: "placement-1",
      cropRegionId: "crop-1",
      x: 100,
      y: 120,
    ...
    expect(auraWindow.setBounds).not.toHaveBeenCalled();
  ...
  it("reloads the current aura profile when add aura mode is requested", async () => {
